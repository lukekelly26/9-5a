...
async function reportPurchaseToMeta({ session, req }) {
  ...
  // If the Pixel isn't configured yet, just skip silently — this should
  // never block or break the actual payment verification.
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (session.payment_status === 'paid') {
      // Fire-and-forget: don't make the customer wait on Meta's API to see their unlocked report.
      reportPurchaseToMeta({ session, req });
